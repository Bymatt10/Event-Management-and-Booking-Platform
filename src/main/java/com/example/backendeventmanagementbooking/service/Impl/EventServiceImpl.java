...
public class EventServiceImpl implements EventService, EventGuestService {
  ...
  public GenericResponse<EventResponseDto> saveEvent(EventDto eventDto) {
    ...
    var categories =
        categoryService.saveOrGetCategoryList(eventDto.getCategories(), savedEvent).stream()
  ...
  @Override
  public GenericResponse<EventResponseDto> updateEvent(UUID uuid, EventUpdatedDto eventUpdatedDto)
    ...
    var listCategories =
        categoryService.updateCategoryNameByEvent(
            eventUpdatedDto.getCategoriesUpdated(), eventEntity);
    ...
    EventEntity eventEntity =
        eventRepository
            ...
            .orElseThrow(
                () ->
                    new CustomException(
                        HttpStatus.NOT_FOUND, "Event not found with UUID: " + uuid));
  ...
  @Override
  public ResponseEntity<GenericResponse<PaginationUtils.PaginationDto<EventResponseDto>>>
      findAllEvents(PageRequest pageRequest) {
    var user = securityTools.getCurrentUser();
    var response =
        pageableRepositoryPaginationDto(eventRepository.findAllByUser(pageRequest, user));
  ...
  public GenericResponse<EventDto> changeDate(UUID uuid, UpdateDateDto updateDate) {
    ...
    eventEntity.setStartDate(updateDate.startDate());
    eventEntity.setEndDate(updateDate.endDate());
    return new GenericResponse<>(
        HttpStatus.OK, objectMapper.convertValue(eventEntity, EventDto.class));
  }

  @Override
  public GenericResponse<EventDto> changeLocation(UUID uuid, String location) {
    EventEntity eventEntity =
        eventRepository
            .findById(uuid)
            .orElseThrow(() -> new CustomException(HttpStatus.NOT_FOUND, "Event not found"));
  ...
  @Override
  public GenericResponse<EventDto> changeCapacity(UUID uuid, int capacity) {
    EventEntity eventEntity =
        eventRepository
            .findById(uuid)
            .orElseThrow(() -> new CustomException(HttpStatus.NOT_FOUND, "Event not found"));
  ...
  @Override
  public ResponseEntity<GenericResponse<PaginationUtils.PaginationDto<EventResponseDto>>>
      listAllMyEventSubscriptions(PageRequest pageRequest) {
    var user = securityTools.getCurrentUser();
    var response =
        pageableRepositoryPaginationDto(
            eventGuestRepository.findAllByUserAndInvitationStatus(user, ACCEPTED, pageRequest));
    var list =
                ...
                eventGuestEntity -> {
                  var converted =
                      objectMapper.convertValue(
                          eventGuestEntity.getEvent(), EventResponseDto.class);
                  var categories =
                      categoryService.getCategoryNameByEvent(eventGuestEntity.getEvent());
  ...
  @Override
  public GenericResponse<PaginationUtils.PaginationDto<EventResponseDto>> searchAllEventsPublic(
      PageRequest pageRequest) {
    var response =
        pageableRepositoryPaginationDto(eventRepository.findAllPublicEvents(pageRequest));
    var dto = paginationFromEntityToEventResponseDto(response);
  ...
  public GenericResponse<EventGuestDto> subscribeToPublicEvent(UUID eventUuid)
    ...
    var user = securityTools.getCurrentUser();
    var event =
        eventRepository.findEventByUuidAndAccessTypeAndStartDateAfter(
            eventUuid, PUBLIC, new Date());
    if (ObjectUtils.isEmpty(event)) return new GenericResponse<>(HttpStatus.NOT_FOUND);

    if (eventGuestRepository.existsByEventAndUser(event, user))
      return new GenericResponse<>(HttpStatus.CONFLICT);

    countParticipantInEvent(event);

    var eventGuest =
        new EventGuestEntity(
            event,
            user,
            PUBLIC,
  ...
  public GenericResponse<Void> unsubscribeFromPublicEvent(UUID eventUuid) {
    ...
    if (event.isEmpty())
      return new GenericResponse<>("Event does not exist!", HttpStatus.NOT_FOUND);
    var eventGuest =
        eventGuestRepository.findByEventAndUserAndInvitationStatus(event.get(), user, ACCEPTED);
    if (eventGuest == null)
      return new GenericResponse<>(
          "User is not subscribe in the current event!", HttpStatus.NOT_FOUND);
  ...
  public GenericResponse<EventGuestDto> subscribeToPrivateEvent(String securityCode)
    ...
    var event =
        eventRepository.findEventByUuidAndAccessTypeAndStartDateAfter(
            eventSaveRedis.eventUuid(), PRIVATE, new Date());
    ...
    countParticipantInEvent(event);
    var eventGuest =
        eventGuestRepository.findByEventAndUserAndInvitationStatus(event, user, PENDING);
    if (eventGuest == null)
      throw new CustomException(HttpStatus.NOT_FOUND, "Event does not exist!");
  ...
  public GenericResponse<Void> unsubscribeFromPrivateEvent(UUID eventUuid) {
    ...
    var getEvent =
        eventRepository.findEventByUuidAndAccessTypeAndStartDateAfter(
            eventUuid, PRIVATE, new Date());
    var eventGuest =
        eventGuestRepository.findByEventAndUserAndInvitationStatus(getEvent, user, PENDING);
    if (eventGuest == null) {
  ...
  private PaginationUtils.PaginationDto<EventResponseDto> paginationFromEntityToEventResponseDto(
      PaginationUtils.PaginationDto<EventEntity> response) {
    var transformation =
        response.getValue().stream()
            .map(
                event -> {
    ...
    return new PaginationUtils.PaginationDto<>(
        transformation,
  ...
  @Async("email")
  protected void sendEmailPrivateEvent(EventEntity event, UserEntity user, String securityCode)
      throws IOException, MessagingException {
    var template =
        buildEmail.getTemplateEmail(
            INVITE_PRIVATE_EVENT,
    ...
    emailSender.sendMail(
        new EmailDetailsDto(user.getEmail(), template, "Invitation to " + event.getTitle()));
  }

  @Async("email")
  protected void sendEmailConfirmation(EventEntity event, UserEntity user)
      throws IOException, MessagingException {
    var template =
        buildEmail.getTemplateEmail(
            CONFIRMATION_EVENT,
